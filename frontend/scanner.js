...
// (Existing functions are kept unchanged)
...
if (captureButton) {
    captureButton.addEventListener('click', async () => {
        ...
        skuInput.value = (nextSku !== null) ? nextSku : '';
        // --- Prepare Payload ---
        const payload = {
            image_data_1: imageDataUrl1,
            image_data_2: imageDataUrl2
        };
        ...
        .then(data => {
            ...
            if (data) {
                 ...
                 // IMPORTANT: Use the price returned in "price" exactly as the database expects.
...
function renderSessionBooks() {
    if (!booksUl || !bookCountSpan) {
         console.error("Book list UL/Span not found.");
         return;
    }
    booksUl.innerHTML = '';
...
if (addBookButton) {
    addBookButton.addEventListener('click', () => {
         ...
         if (isNaN(qtyInt) || qtyInt < 1) { alert("Qty must be >= 1."); return; }
         ...
         const bookData = {
              ...
              cond_text: condText,
              edition: edition,
              signature: isSigned ? "Signed" : "",
              language: language
         };
         sessionBooks.push(bookData);
         console.log("Book added:", bookData);
         console.log("Session Array:", sessionBooks);
         renderSessionBooks();
...
function exportBooksToCsv() {
    ...
    const headers = ['sku', 'location', 'cost', 'source', 'isbn', 'title', 'author', 'publisher', 'release_date', 'image', 'media', 'price', 'condition', 'notes', 'qty', 'cond_text', 'edition', 'signature', 'language'];
