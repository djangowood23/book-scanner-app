...
// --- Global Variables ---
let sessionBooks = [];
let lastBackendResponseData = null; // Still useful for debugging backend response
...
// Input field references (keep all for Add Book / Populate)
...
if (captureButton) {
    captureButton.addEventListener('click', () => { // No longer async
        ...
        // --- Capture Image 1 ONLY ---
        canvas.width = video.videoWidth; canvas.height = video.videoHeight;
        context.drawImage(video, 0, 0, canvas.width, canvas.height);
        const imageDataUrl1 = canvas.toDataURL('image/jpeg', 0.9);
        snapshotImg.src = imageDataUrl1;
        snapshotImg.style.display = 'block';
        console.log("Snapshot taken.");

        // --- Remove prompt and second capture logic ---

        // --- Prepare Payload (Only Image 1) ---
        const payload = {
            image_data_1: imageDataUrl1 // Only send image 1
            // No image_data_2
        };
        ...
        .then(data => {
             ...
             lastBackendResponseData = data; // Store response
             ...
             if (data) {
                  ...
                  // Populate fields based on backend response
             ...
             alert("Image processed! Review details and click 'Add Book'."); // Simplified alert
        })
        .catch(error => { /* Keep existing error handling */ console.error('Error sending image to backend:', error); alert(`Failed to process image. Error: ${error.message}. Check browser console.`); });
    });
