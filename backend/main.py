...
from dotenv import load_dotenv # For local .env testing
...
if not GCS_BUCKET_NAME:
    logging.warning("GCS_BUCKET_NAME environment variable not set. Upload will likely fail.")


# Decorate the main function directly as the HTTP entry point
@functions_framework.http
def handle_process_image(request):
    """Handles OPTIONS and POST: Decodes, uploads, calls Vision, parses text."""

    # --- Handle OPTIONS preflight request ---
    if request.method == 'OPTIONS':
        logging.info("Handling OPTIONS request")
        # Return tuple (body, status, headers) is standard for functions-framework >= 3.0
    ...
    # --- Handle POST request ---
        ...
        response_headers['Content-Type'] = 'application/json' # Default to JSON response
        ...
        try:
            ...
            try:
                ...
                if not data or 'image_data' not in data:
                    logging.error("Missing or invalid JSON/image_data in POST request")
                    return ({'error': 'Missing or invalid image_data in JSON payload'}, 400, response_headers)

                image_data_url = data['image_data']
                if ';base64,' not in image_data_url:
                     logging.error(f"Invalid image data format: {image_data_url[:50]}...")
                     return ({'error': 'Invalid image data format, expected base64 data URL'}, 400, response_headers)
            ...
            if image_bytes and GCS_BUCKET_NAME:
                try:
                    ...
                    logging.exception(gcs_error) # Log full traceback
            ...
            if filename: # Only call Vision if GCS upload seemed ok (filename assigned)
                try:
                    ...
                    if response.error.message:
                        ...
                        extracted_text = "" # Handle case where Vision finds no text
                except Exception as vision_api_error:
                    vision_error = f"Vision API Call Failed: {vision_api_error}"
                    logging.exception(vision_error)
            else:
                 vision_error = "Skipping Vision API call because GCS upload failed or was skipped."
                 logging.warning(vision_error)

            # 4. Basic Parsing Logic
            if extracted_text: # Check if text exists to parse
                ...
                for i, line in enumerate(lines):
                    ...
                    if line_lower.strip() == 'by' or ' by ' in line_lower:
                         if i + 1 < len(lines) and lines[i+1] != parsed_isbn: # Check next line
                             parsed_author = lines[i+1]
                             author_line_index = i+1
                             logging.info(f"Found potential Author: {parsed_author}")
                             break
                    elif i > 0 and lines[i-1].lower().endswith(' by'): # Check end of previous line
                ...
                for i, line in enumerate(lines):
                    # *** FIX FOR TypeError ***
                    # Check if parsed_isbn is None before using 'in'
            ...
            elif vision_error is None: # Log only if no prior vision error
            ...
            response_data = {
                "message": "Image processed successfully.", # More positive message
                "image_url": image_url,
                "gcs_error": gcs_error, # Include potential GCS error
                "vision_error": vision_error, # Include potential Vision error
    ...
    # --- Handle other methods ---
        ...
        # Also add CORS header to this error response
