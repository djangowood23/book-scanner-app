...
def handle_process_image(request):
    ...
    # --- Handle POST request ---
        ...
        filename_1 = None
        filename_2 = None
        gcs_error = None # Combined GCS error reporting
        gemini_error = None
        ...
        try:
            ...
            if image_bytes_1 and GCS_BUCKET_NAME:
                try:
                    ...
                    gcs_error = f"GCS Upload Error: {err}" # General GCS error
                    logging.exception(gcs_error)
                    if not image_url_1: filename_1 = None # Ensure filename is None if primary upload failed
            ...
            if image_bytes_1 and gemini_configured:
                try:
                    ...
                    if image_bytes_2:
                        ...
                        logging.info(f"Sending 2 images to Gemini.")
                    else:
                        logging.info(f"Sending 1 image to Gemini.")
                    ...
                    try:
                        ...
                        if json_start != -1 and json_end != -1 and json_end > json_start:
                             ...
                             raise ValueError(f"No valid JSON object found in response text: {response_text[:500]}") # Include snippet
            ...
            # Decide on primary image URL - use image_url_1 (cover/first shot)
            ...
            response_data = {
                ...
                "gcs_error": gcs_error, # Use combined GCS error
